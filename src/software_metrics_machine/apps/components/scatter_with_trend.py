...
def _to_numeric_x(series: pd.Series) -> Tuple[pd.Series, bool]:
...
def _normalize_to_rows(data: Any, x: str, y: str) -> List[dict]:
...
def build_scatter_with_trend(
    ...
    """
    rows = _normalize_to_rows(data, x, y)

    if not rows:
        return _empty_figure("No data available", height=height)
    ...
    numeric_x_for_color, _is_dt_color = _to_numeric_x(x_series_for_color)
    ...
    if categorical:
        ...
        palette = (
            Category10[10] if len(factors) > 10 else Category10[max(3, len(factors))]
        )
        MARKERS = [
            "hex",
            "circle_x",
        ...
        p = figure(title=title or "", background_fill_color="#fafafa", height=height)
    ...
    if add_color:
        ...
        pts = pts.opts(color="#1f77b4", **({"tools": tools} if tools else {}), **p_opts)
