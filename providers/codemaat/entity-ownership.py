...
top_entities = ownership.groupby('entity')['total'].sum().sort_values(ascending=False).head(MAX_ENTITIES).index
...
pivot_deleted = ownership.pivot(index='entity', columns='author', values='deleted').fillna(0)
...
for author in pivot_deleted.columns:
	ax.bar(pivot_deleted.index, pivot_deleted[author], label=f"{author} deleted", bottom=bottom_deleted, alpha=0.5)
