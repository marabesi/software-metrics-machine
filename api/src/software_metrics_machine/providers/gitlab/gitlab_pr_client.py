...
class GitlabPrsClient:
    ...
    def fetch_prs(
        ...
        url = f"https://gitlab.com/api/v4/projects/{self.encoded_project}/merge_requests"
        ...
        while url and not stop:
            ...
            for pr in page_prs:
                # GitLab uses created_at or created_on depending on endpoint; try both
                created_str = pr.get("created_at") or pr.get("created_on")
                if not created_str:
                    continue
            ...
            # GitLab pagination uses headers; requests exposes links similarly for common patterns
    ...
    def fetch_pr_comments(
        ...
        comments = []
        for pr in prs:
            # Prefer an explicit review_comments_url if provided (keeps parity with Github client)
            notes_url = getattr(pr, "review_comments_url", None)
            if not notes_url:
                # GitLab notes endpoint for merge requests: use iid if available
                mr_iid = pr.iid if hasattr(pr, "iid") else pr.number if hasattr(pr, "number") else None
                if mr_iid:
                    notes_url = f"https://gitlab.com/api/v4/projects/{self.encoded_project}/merge_requests/{mr_iid}/notes"

            if notes_url:
                print(f"Fetching review comments from {notes_url}")
                params = {
                    "per_page": "100",
                }
                if raw_params:
                ...
                r = requests.get(notes_url, headers=self.HEADERS, params=params)
                ...
                print("No merge request IID found on PR")
