...
if ! git log --pretty=format:'[%h] %aN %ad %s' --date=short --numstat --after="$start_date" $target_directory > "$store_data/$git_log_file"; then
  echo "❌ Failed to generate git log file at $store_data/$git_log_file"
  exit 1
fi

cd "$current"

if [ ! -s "$store_data/$git_log_file" ]; then
  echo "❌ logfile.log was not created or is empty at $store_data/$git_log_file"
  exit 1
fi
